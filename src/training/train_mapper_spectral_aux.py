"""Fresh layer-9 plus distorted spectral input experiment. Separate outputs; never reads test audio."""
import argparse
import csv
import gc
import hashlib
...
from src.models.spectral_aux_mapper import SpectralAuxMapper, extract_log_mel, MEL_CONFIG
...
def main(args):
    ...
    if args.max_steps > 500:
        raise ValueError('This smoke trainer is bounded to at most 500 steps')
    out = (ROOT/args.output_dir).resolve()
    allowed = ROOT/'results/checkpoints/spectral_aux_mapper'
    if not out.is_relative_to(allowed) or out == allowed:
        raise ValueError('Use a NEW child of results/checkpoints/spectral_aux_mapper')
    ...
    provenance = dict(args=vars(args), model_config=cfg, optim=optim_cfg, seed=42,
                      ...
                      initialization='fresh mapper; no pretrained mapper weights', mel_config=MEL_CONFIG, gradient_clip=args.grad_clip,
                      alignment='training/val L1: clean mel length, like original; audio inference: distorted length',
                      cache='reuse frozen FP16 cache: select ONLY hidden_states[9]; no trainable layer mixing',
    ...
    targets, frames, cache_paths, spectral = {}, {}, {}, {}
    encoder = None
    for i, row in enumerate(rows, 1):
        ...
        targets[uid] = extract_log_mel(clean.mean(axis=1), sr)
        distorted, sr = sf.read(source, dtype='float32', always_2d=True)
        spectral[uid] = extract_log_mel(distorted.mean(axis=1), sr)
    ...
    def batch(rr):
        hs = [(torch.from_numpy(np.load(cache_paths[r['utterance_id']], mmap_mode='r')[9].copy()).to(device),
               spectral[r['utterance_id']].to(device)) for r in rr]
    ...
    set_seed(42)
    model = SpectralAuxMapper(cfg).to(device)
    ...
    def validation(step, use_old=False, audio=True):
        ...
        for start in range(0, len(split['val']), 4):
            ...
            if use_old:
                ee = [torch.nn.functional.interpolate(h[0].float().T[None], size=int(n), mode='linear', align_corners=False)[0].T for h,n in zip(hs,lengths)]
        ...
        if audio:
            ...
            for row in val_audio:
                ...
                if use_old:
                    emb=torch.nn.functional.interpolate(hs[0][0].float().T[None],size=n,mode='linear',align_corners=False).transpose(1,2)
                    pred=old(emb)
    ...
    losses=[]; times=[]; step=0; epoch=0; best=float('inf'); clipped_steps=0; max_gradient_norm=0.
    while step < args.max_steps:
        ...
        for start in range(0,len(order),bs):
            ...
            clipped_steps += int(float(norm) > args.grad_clip)
            ...
            entry=dict(step=step,epoch=epoch,mel_l1=losses[-1],gradient_norm=float(norm),gradient_clipped=float(norm)>args.grad_clip,seconds=times[-1],lr=optimizer.param_groups[0]['lr'])
        ...
        if val['val_mel_l1']<best:
            best=val['val_mel_l1'];torch.save(dict(step=step,model=model.state_dict(),model_config=model.model_config),out/'best.pt')
    torch.save(dict(step=step,epoch=epoch,model=model.state_dict(),model_config=model.model_config,optimizer=optimizer.state_dict(),
    ...
    summary=dict(steps=step,epochs=epoch,all_finite=True,first_50_loss=float(np.mean(losses[:50])),last_50_loss=float(np.mean(losses[-50:])),
                 mean_step_seconds=float(np.mean(times)),clipped_steps=clipped_steps,max_gradient_norm=max_gradient_norm,
                 baseline=baseline_result,validation=trend,best_val_l1=best,
